...
public class DelayedBaker_RopeFence extends DelayedBaker implements ICustomModelLoader{
	...
	@Override
	public Collection<ResourceLocation> getTextures() { return DelayedBaker_RopeFence.textures; }
...
//		IExtendedBlockState state = (IExtendedBlockState) st;
		...
		int north = state.getValue(RopeFence.NORTH);
		int east = state.getValue(RopeFence.EAST);
		int south = state.getValue(RopeFence.SOUTH);
		int west = state.getValue(RopeFence.WEST);
	...
	public boolean accepts(ResourceLocation modelLocation) {
		return ((modelLocation instanceof ModelResourceLocation) && DRPMedievalBlocks.ROPE_FENCE.getRegistryName().equals(modelLocation) && !modelLocation.getResourcePath().contains("inventory"));
	}
	
	protected static IModel getModel(String modelName){
		return ModelLoaderRegistry.getModelOrLogError(new ResourceLocation(DRPMedievalInfo.MODID, "block/rope_fence/" + modelName), "A problem occured while trying to load: " + DRPMedievalInfo.MODID + ":block/rope_fence/" + modelName);
	}
}
