...
import net.dark_roleplay.medieval.objects.blocks.building.timbered_clay.TimberedClay.ClickInfo;
import net.dark_roleplay.medieval.objects.blocks.building.timbered_clay.TimberedClay.ClickLoc;
import net.dark_roleplay.medieval.objects.blocks.building.timbered_clay.TimberedClay.TimberRecipe;
...
public class TimberedClayHud extends Hud{
	...
	public void render(int width, int height, float partialTicks) {
		...
		if(TimberedClay.clicks.containsKey(Minecraft.getMinecraft().player)) {
			ClickInfo info = TimberedClay.clicks.get(Minecraft.getMinecraft().player);

			int baseX = this.alignment.getBaseX(width, 0) + this.posX;
			int baseY = this.alignment.getBaseY(height, 0) + this.posY;

			IBlockState state = Minecraft.getMinecraft().world.getBlockState(info.getPos());
