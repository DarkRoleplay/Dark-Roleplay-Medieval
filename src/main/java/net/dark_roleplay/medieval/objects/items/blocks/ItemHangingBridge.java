...
public class ItemHangingBridge extends DRPItem{
			...
			if(dist > 65){
				...
				for(int i = 1; i < (length); i ++){
					...
					if(posX1 == posX2){
						...
						if(((drop % 16) / 8) == 0){
							world.setBlockState(placePos, state = MedievalBlocks.HANGING_BRIDGE_TOP.getDefaultState().withProperty(MedievalBlockProperties.AXIS_HORIZONTAL, EnumFacing.Axis.X).withProperty(HangingBridge.HEIGHT, 7 - Math.abs(drop % 8)));
						}else{
							world.setBlockState(placePos, state = MedievalBlocks.HANGING_BRIDGE_BOTTOM.getDefaultState().withProperty(MedievalBlockProperties.AXIS_HORIZONTAL, EnumFacing.Axis.X).withProperty(HangingBridge.HEIGHT, 7 - Math.abs(drop % 8)));
						}
						world.markAndNotifyBlock(placePos, world.getChunk(placePos), world.getBlockState(placePos), state, 3);
					}else{
						placePos = posX1 > posX2 ? pos2.add(i,-Math.abs(drop / 16),0) : pos.add(i, -Math.abs(drop / 16),0);
						if(((drop % 16) / 8) == 0){
							world.setBlockState(placePos, state =  MedievalBlocks.HANGING_BRIDGE_TOP.getDefaultState().withProperty(MedievalBlockProperties.AXIS_HORIZONTAL, EnumFacing.Axis.Z).withProperty(HangingBridge.HEIGHT, 7 - Math.abs(drop % 8)));
						}else{
							world.setBlockState(placePos, state =  MedievalBlocks.HANGING_BRIDGE_BOTTOM.getDefaultState().withProperty(MedievalBlockProperties.AXIS_HORIZONTAL, EnumFacing.Axis.Z).withProperty(HangingBridge.HEIGHT, 7 - Math.abs(drop % 8)));
						}
						world.markAndNotifyBlock(placePos, world.getChunk(placePos), world.getBlockState(placePos), state, 3);
					}
				}
