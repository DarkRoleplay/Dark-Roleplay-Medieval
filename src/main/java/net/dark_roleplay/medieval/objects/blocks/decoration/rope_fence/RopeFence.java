...
import net.dark_roleplay.medieval.objects.block_properties.UnlistedPropertyInteger;
...
public class RopeFence extends Block {
	...
	public RopeFence(String registryName) {
		...
		this.setDefaultState(this.getDefaultState().withProperty(NORTH_EAST, false).withProperty(NORTH_WEST, false)
	...
	protected BlockStateContainer createBlockState() {
		return new ExtendedBlockState(this,
				new IProperty[] { RopeFence.NORTH_EAST, RopeFence.SOUTH_EAST, RopeFence.SOUTH_WEST,
						RopeFence.NORTH_WEST },
				new IUnlistedProperty[] { RopeFence.NORTH, RopeFence.EAST, RopeFence.WEST, RopeFence.SOUTH });
	}
	...
	public IBlockState getExtendedState(IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		if (state instanceof IExtendedBlockState) {
			...
			ext = (IExtendedBlockState) ext.withProperty(RopeFence.NORTH, n).withProperty(RopeFence.EAST, e)
					.withProperty(RopeFence.SOUTH, s).withProperty(RopeFence.WEST, w)
					.withProperty(RopeFence.NORTH_EAST, ne).withProperty(RopeFence.SOUTH_WEST, sw)
					.withProperty(RopeFence.SOUTH_EAST, se).withProperty(RopeFence.NORTH_WEST, nw);
			;
			state = ext;
		}
	...
	@Override
	public void addCollisionBoxToList(IBlockState unextended, World worldIn, BlockPos pos, AxisAlignedBB entityBox,
		...
		if (entity instanceof EntityPlayer) {
			...
			if (north != 3) {
				Block.addCollisionBoxToList(pos, entityBox, collidingBoxes,
						RopeFence.NORTH_AABB.contract(0D, 0.5D, 0D));
			}
			...
			if (south != 3) {
				Block.addCollisionBoxToList(pos, entityBox, collidingBoxes,
						RopeFence.SOUTH_AABB.contract(0D, 0.5D, 0D));
			}
