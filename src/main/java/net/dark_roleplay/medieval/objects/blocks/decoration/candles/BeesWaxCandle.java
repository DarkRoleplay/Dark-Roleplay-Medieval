...
public class BeesWaxCandle extends Block{
	...
	public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand){
		if(!state.getValue(MedievalBlockProperties.LIT))
			return;
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ){
		if(!world.isRemote){
			boolean lit = state.getValue(MedievalBlockProperties.LIT);
			if(!lit && player.getHeldItem(hand).getItem() == Items.FLINT_AND_STEEL){
				world.setBlockState(pos, state.withProperty(MedievalBlockProperties.LIT, true));
				player.getHeldItem(hand).damageItem(1, player);
				return true;
			}else if(lit && player.getHeldItem(hand).isEmpty() && player.isSneaking()){
				world.setBlockState(pos, state.withProperty(MedievalBlockProperties.LIT, false));
				return true;
			}
		}
	...
	public int getLightValue(IBlockState state){
		return state.getValue(MedievalBlockProperties.LIT) ? 15 : 0;
	}

	@Override
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(MedievalBlockProperties.LIT, meta == 1 ? true : false);
	}

	@Override
	public int getMetaFromState(IBlockState state) {
		return state.getValue(MedievalBlockProperties.LIT) ? 1 : 0;
	}

	@Override
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] {MedievalBlockProperties.LIT});
	}

	@Override
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer){
		return this.getDefaultState().withProperty(MedievalBlockProperties.LIT, false);
	}
}