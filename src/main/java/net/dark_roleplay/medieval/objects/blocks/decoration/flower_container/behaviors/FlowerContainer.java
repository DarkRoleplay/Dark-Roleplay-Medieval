package net.dark_roleplay.medieval.objects.blocks.decoration.flower_container.behaviors;
...
public class FlowerContainer implements IActivatedBehavior{
	...
	public boolean execute(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if(player.getHeldItem(hand).getItem() instanceof ItemBlock) {
			...
			if(!(te instanceof TileEntityFlowerContainer)) return false;

			TileEntityFlowerContainer flower = (TileEntityFlowerContainer) te;
