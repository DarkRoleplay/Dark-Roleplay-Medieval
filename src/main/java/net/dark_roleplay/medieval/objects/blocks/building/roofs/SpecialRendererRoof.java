...
//	}
	...
	public void renderTileEntityFast(TileEntityRoof te, double x, double y, double z, float partialTicks, int destroyStage, float partial, BufferBuilder buffer) {
		...
		EnumFacing facing = world.getBlockState(pos).getValue(MedievalBlockProperties.FACING_HORIZONTAL);

		TextureAtlasSprite sprite = north.get(otherState);
//		if(this.model == null) {
			this.model = new BakedRoof(facing, sprite, world.getBlockState(pos).getActualState(world, pos).getValue(MedievalBlockProperties.STAIR_TYPE));
//		}
