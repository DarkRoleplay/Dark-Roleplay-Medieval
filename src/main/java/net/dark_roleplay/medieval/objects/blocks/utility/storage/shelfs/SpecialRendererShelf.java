...
public class SpecialRendererShelf  extends TileEntitySpecialRenderer<TileEntityShelf> {
	...
	public SpecialRendererShelf(RenderItem itemRenderer) {
	...
	public void render(TileEntityShelf te, double x, double y, double z, float partialTicks, int destroyStage, float alpha){
		...
		EnumFacing facing = (EnumFacing) state.getValue(MedievalBlockProperties.FACING_HORIZONTAL);
	...
	public class ItemMap{
		...
		public void refresh(ItemStack stack, World world){
			if(this.stack.equals(stack)){
				...
				this.model = SpecialRendererShelf.this.itemRenderer.getItemModelWithOverrides(stack, world, (EntityLivingBase)null);
				this.stack = stack;
			}
		}
		...
		public void render(){
			if(this.stack != null && this.model != null){
				...
				SpecialRendererShelf.this.itemRenderer.renderItem(this.stack, ForgeHooksClient.handleCameraTransforms(this.model, ItemCameraTransforms.TransformType.NONE, false));
				GL11.glPopMatrix();
			}
		}
	}
