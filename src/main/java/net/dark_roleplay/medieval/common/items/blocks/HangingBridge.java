...
public class HangingBridge extends Item{
			...
			if(dist > 65){
				...
				for(int i = 1; i < (length); i ++){
					...
					if(posX1 == posX2){
						...
						if(((drop % 16) / 8) == 0){
							world.setBlockState(placePos, state = DRPMedievalBlocks.HANGING_BRIDGE_TOP.getDefaultState().withProperty(HangingBridge2.AXIS, EnumAxis.X).withProperty(HangingBridge2.HEIGHT, 7 - Math.abs(drop % 8)));
						}else{
							world.setBlockState(placePos, state = DRPMedievalBlocks.HANGING_BRIDGE_BOTTOM.getDefaultState().withProperty(HangingBridge2.AXIS, EnumAxis.X).withProperty(HangingBridge2.HEIGHT, 7 - Math.abs(drop % 8)));
						}
						world.markAndNotifyBlock(placePos, world.getChunkFromBlockCoords(placePos), world.getBlockState(placePos), state, 3);
					}else{
						placePos = posX1 > posX2 ? pos2.add(i,-Math.abs(drop / 16),0) : pos.add(i, -Math.abs(drop / 16),0);
						if(((drop % 16) / 8) == 0){
							world.setBlockState(placePos, state =  DRPMedievalBlocks.HANGING_BRIDGE_TOP.getDefaultState().withProperty(HangingBridge2.AXIS, EnumAxis.Z).withProperty(HangingBridge2.HEIGHT, 7 - Math.abs(drop % 8)));
						}else{
							world.setBlockState(placePos, state =  DRPMedievalBlocks.HANGING_BRIDGE_BOTTOM.getDefaultState().withProperty(HangingBridge2.AXIS, EnumAxis.Z).withProperty(HangingBridge2.HEIGHT, 7 - Math.abs(drop % 8)));
						}
						world.markAndNotifyBlock(placePos, world.getChunkFromBlockCoords(placePos), world.getBlockState(placePos), state, 3);
					}
				}
