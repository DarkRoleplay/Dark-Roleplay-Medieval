...
public class SpecialRendererBarrel extends TileEntitySpecialRenderer<TileEntityBarrel>{
	...
	@Override
	public void render(TileEntityBarrel tileEntityBarrel, double x, double y, double z, float partialTicks, int destroyStage, float alpha){

		if(tileEntityBarrel.getStorageType() != StorageType.FLUID) return;
		IBlockState state = tileEntityBarrel.getWorld().getBlockState(tileEntityBarrel.getPos());
		if(!(state.getBlock() instanceof Barrel) || state.getValue(MedievalBlockProperties.IS_CLOSED)) return;

		FluidTank fluidTank = (FluidTank) tileEntityBarrel.getCapability(CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY, null);
