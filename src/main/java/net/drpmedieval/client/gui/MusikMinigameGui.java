...
public class MusikMinigameGui extends DRPGuiScreen implements ITimedGui{
		...
		for(int i=0; i < this.notes.size(); i++){
			...
			switch(nt.getType()){
				...
				case 2:
					...
					break;
				case 3:
					notePositions.add( i-1 > 0 ? notePositions.get(i-1) + 10 : +0);
					break;
				default:
					notePositions.add( i-1 > 0 ? notePositions.get(i-1) + 20 : +0);
					break;
	...
	protected void drawForeground(int arg0, int arg1, float arg2) {
	    ...
	    for(int i = 0; i < notes.size(); i++){
	    	Note nt = notes.get(i);
	    	if((currentTick * 5) + ( 100 * currentNote) - 20 - notePositions.get(i) > -20)
		    this.drawTexturedModalRect(bX + (int)((nt.getLane()+ 0.5) * noteWidth) - 3, (currentTick * 5) + ( 100 * currentNote) - 20 - notePositions.get(i),120,0, 10, 16);
	    }
	}
    ...
    protected void keyTyped(char typedChar, int keyCode, boolean down) throws IOException{
        if (keyCode == 2){
        	if(down){
        		key1 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 1.0f, true);
        	}else{
        		key1 = false;
        	}
        }if (keyCode == 3){
        	if(down){
        		key2 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 0.9f, true);
        	...
        	if(down){
        		key3 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 0.8f, true);
        	...
        	if(down){
        		key4 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 0.7f, true);
        	...
        	if(down){
        		key5 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 0.6f, true);
        	...
        	if(down){
        		key6 = true;
        		Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.getPosition().add(0.5,0.5,0.5), SoundEvents.BLOCK_NOTE_HARP, SoundCategory.PLAYERS, 1f, 0.5f, true);
