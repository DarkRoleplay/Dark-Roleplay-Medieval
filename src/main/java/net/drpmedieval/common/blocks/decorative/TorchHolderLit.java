...
public class TorchHolderLit extends Block{
	...
	public TorchHolderLit() {
		...
		this.setUnlocalizedName("blockTorchHolderLit");
	    	...
	    	if(!worldIn.isRemote){
	    		if(playerIn.getHeldItem() == null){
	    			if((Boolean)state.getValue(AddonTrap) && !(Boolean)state.getValue(POWERED)){
			           ...
			           worldIn.scheduleUpdate(pos, DRPMedievalBlocks.torchHolderUnlit, 60);
	    			...
	    			if(playerIn.isSneaking()){
		    			worldIn.setBlockState(pos, DRPMedievalBlocks.torchHolderUnlit.getDefaultState().withProperty(FACING, state.getValue(FACING)).withProperty(AddonLighter, state.getValue(AddonLighter)).withProperty(AddonTrap, state.getValue(AddonTrap)).withProperty(POWERED, state.getValue(POWERED)));
	    			}
