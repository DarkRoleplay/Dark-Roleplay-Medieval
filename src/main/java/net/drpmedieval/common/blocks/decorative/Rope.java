			...
			if(player.getHeldItem().getItem().equals(Item.getItemFromBlock(DRPMedievalBlocks.rope))){
				for(int i = pos.getY() - 1; i > 0; i--){
					...
					if(worldIn.getBlockState(pos2).getBlock().equals(DRPMedievalBlocks.rope)){
						...
						player.inventory.consumeInventoryItem(Item.getItemFromBlock(DRPMedievalBlocks.rope));
						return true;
					}else{
						return true;
					}
			...
			if(player.isSneaking()){
				for(int i = pos.getY() - 1; i > 0; i--){
					...
					if(worldIn.getBlockState(pos2).getBlock().equals(DRPMedievalBlocks.rope)){
						...
						player.worldObj.spawnEntityInWorld(new EntityItem(player.worldObj,player.posX,player.posY,player.posZ, new ItemStack(DRPMedievalBlocks.rope,1)));
						return true;
					}
				}
			}
