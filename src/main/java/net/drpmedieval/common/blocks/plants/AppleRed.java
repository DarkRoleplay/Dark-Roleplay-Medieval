...
public class AppleRed  extends Block{
	...
	public AppleRed() {
		...
		this.setUnlocalizedName("blockAppleRed");
	...
	//Growing Start
	    ...
	    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side, float hitX, float hitY, float hitZ)
	    {
	    	if(!worldIn.isRemote){
	    		if(Collections.max(AGE.getAllowedValues()) - 2 == state.getValue(AGE).intValue()){
	    			worldIn.spawnEntityInWorld(new EntityItem(worldIn, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(Items.apple,1)));
